...
(function() {
  ...
  ensureObject('Trait', './traits.js', ['Trait']);
  ...
  strategies = {
    compose: function(adaptation, trait) {
      ...
      console.log(resultingTrait);
      console.log(adaptation.trait);
      console.log(trait);
      for (name in resultingTrait) {
        if (!__hasProp.call(resultingTrait, name)) continue;
        propdesc = resultingTrait[name];
        if (propdesc.conflict) {
          var err = "Property '" + name + "' already adapted for the object " + JSON.stringify(adaptation.object, null, 2) + " in context " + adaptation.context.name();
          throw new Error(err);
        }
      }
